...
const HIDE_SUGGESTIONS_AFTER = 2
const GRAY_OUT_AFTER = 7
...
export default function AuthModal() {
  ...
  const isSearchDisabled = searchAttempts.current >= GRAY_OUT_AFTER
  const showSuggestions = searchAttempts.current < HIDE_SUGGESTIONS_AFTER
  ...
  const handleSearch = useCallback((e) => {
    ...
    if (searchAttempts.current >= MAX_SEARCH_ATTEMPTS) return
  ...
  }, [searchTerm, searchGuests, isSearchDisabled])

  const handleTermChange = useCallback((e) => {
    const val = e.target.value
    setSearchTerm(val)
    if (val.length >= 2 && showSuggestions) {
  ...
  }, [searchGuests, showSuggestions])

  const handleOAuth = useCallback(async (provider) => {
    if (!clerkLoaded || !clerkSignIn) {
      setOauthError(`${provider} is not configured. Add it in your Clerk dashboard.`)
      return
    }
    ...
    try {
      ...
      setOauthError(`${provider} sign-in is not configured yet.`)
      setOauthLoading(null)
    }
                ...
                <>
                  ...
                  {!isSearchDisabled ? (
                    <form onSubmit={handleSearch} className="mb-6">
                      ...
                      {searched && results.length > 0 && showSuggestions && (
                        <div className="mt-2 border border-gold/10 rounded-sm bg-cream-dark max-h-48 overflow-y-auto">
                          {results.map((g) => (
                            <button
                              key={g.id}
                              type="button"
                      ...
                      {searched && results.length === 0 && searchTerm.length >= 2 && (
                        <p className="mt-2 text-xs text-charcoal-light/40 text-center">No matches found</p>
                      )}

                      {searchAttempts.current > 0 && (
                        <p className="mt-2 text-xs text-charcoal-light/30 text-center">
                          {MAX_SEARCH_ATTEMPTS - searchAttempts.current} searches remaining
                        </p>
                      )}
                    </form>
                  ) : (
